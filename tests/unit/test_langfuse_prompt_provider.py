...
@pytest.mark.unit
async def test_prompt_provider_gets_prompt_successfully(
    ...
    fake_client.create_prompt(
        ...
        prompt="Generate {count} questions for {topic}",
    )
    ...
    assert prompt.name == "Assessment Generator"
    assert prompt.version == 1
    assert prompt.is_chat_prompt() is False
...
@pytest.mark.unit
async def test_prompt_provider_handles_not_found(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fake_client = FakeLangfuseClient(
        public_key="pk-test",
